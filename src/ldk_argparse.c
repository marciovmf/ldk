...
{
  printf("Usage:\n");
  for (u32 i = 0; i < parser->argCount; i++)
  {
...
void ldkArgParseArguments(LDKArgParser *parser, u32 argc, char *argv[])
{
  for (u32 i = 1; i < argc; i++)
  {
    ...
    for (u32 j = 0; j < parser->argCount; j++)
    {
...
i32 ldkArgGetIntValue(const LDKArgParser *parser, const char *name)
{
  for (u32 i = 0; i < parser->argCount; i++)
  {
