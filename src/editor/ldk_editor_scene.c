...
{
  LDKSceneEntityMapEntry *new_entries = NULL;
  u32 new_capacity = 0;
  ...
  new_capacity = map->capacity ? map->capacity * 2u : 64u;
  while (new_capacity < min_capacity)
  {
    new_capacity *= 2u;
  }

  new_entries = (LDKSceneEntityMapEntry *)realloc(
      map->entries, sizeof(LDKSceneEntityMapEntry) * new_capacity);
...
static bool s_map_push(LDKSceneEntityMap *map, LDKEntity entity, i32 scene_id)
{
    ...
    const LDKSceneEntityMap *map, LDKEntity entity, i32 *out_scene_id)
{
  ...
  {
    if (s_entity_eq(map->entries[i].entity, entity))
    {
...
{
  LDKScenePendingParent *new_entries = NULL;
  u32 new_capacity = 0;
  ...
  new_capacity = list->capacity ? list->capacity * 2u : 64u;
  while (new_capacity < min_capacity)
  {
    new_capacity *= 2u;
  }

  new_entries = (LDKScenePendingParent *)realloc(
      list->entries, sizeof(LDKScenePendingParent) * new_capacity);
...
static bool s_tml_string_eq(TMLString string, const char *cstr)
{
  ...
  len = strlen(cstr);
  return string.size == (u32)len && memcmp(string.data, cstr, len) == 0;
}
    ...
    const char *name, u32 *out_value)
{
  ...
  if (!entry || !tml_entry_get_i64(entry, &value))
  {
    return false;
  }
...
static void s_append_escaped_string(XStrBuilder *out, const char *text)
{
  ...
  if (p)
  {
    while (*p)
    {
      ...
      default:
        x_strbuilder_append_char(out, *p);
        break;
      }
...
static bool s_write_scene_entity_reference(
    XStrBuilder *out, const LDKSceneEntityMap *map, LDKEntity entity)
{
...
static bool s_write_field_value(XStrBuilder *out,
    const LDKSceneEntityMap *entity_map, const LDKComponentFieldMeta *field,
    const void *component)
{
  const u8 *base = (const u8 *)component;
  const void *ptr = base + field->offset;
  ...
  case LDK_FIELD_VEC4:
  {
    ...
    x_strbuilder_append_format(out, "%.9g, %.9g, %.9g, %.9g", (double)value->x,
        (double)value->y, (double)value->z, (double)value->w);
  }
  break;
  ...
  case LDK_FIELD_ENTITY:
  {
    ...
    if (!s_write_scene_entity_reference(out, entity_map, *value))
    {
      return false;
    }
  }
  ...
  case LDK_FIELD_ASSET_MESH:
  {
    ...
    x_strbuilder_append_format(out, "%d", asset_id);
  }
  break;
...
static bool s_read_f32_array(const TMLDocument *doc, const TMLEntry *entry,
    float *out_values, u32 expected_count)
{
  u32 i = 0;
...
static bool s_read_scene_entity_reference(
    const LDKSceneEntityMap *map, const TMLEntry *entry, LDKEntity *out_entity)
{
  i32 scene_id = LDK_SCENE_NULL_ENTITY_ID;
  ...
  if (!s_entry_get_i32(entry, &scene_id))
  {
    return false;
  }
...
static bool s_apply_field_value(const TMLDocument *doc, const TMLEntry *entry,
    const LDKSceneEntityMap *entity_map, const LDKComponentFieldMeta *field,
    void *component)
{
  u8 *base = (u8 *)component;
...
static bool s_write_component(LDKSceneSaveContext *context, LDKEntity entity,
    u32 component_type, bool *out_wrote_any)
{
  const LDKComponentMeta *meta = NULL;
  const void *component = NULL;
  u32 i = 0;
  bool wrote_any_field = false;

  meta = ldk_scene_component_meta_find_by_type(context->game, component_type);
  if (!meta)
  {
    /*
     * No metadata means we cannot safely serialize the component data.
     * Skip it rather than guessing its binary layout.
     */
    return true;
  }
  ...
  for (i = 0; i < meta->field_count; ++i)
  {
    ...
    if (!s_write_field_value(context->out, &context->map, field, component))
    {
      s_result_error_format(context->result,
          "failed to serialize component field: %s.%s",
          meta->name,
          field->name);
      return false;
    }
...
static bool s_write_entity_callback(LDKEntity entity, void *user)
{
  ...
  u32 i = 0;
  bool wrote_any_component = false;
  ...
  {
    if (!s_map_find_id_by_entity(&context->map, parent, &parent_id))
    {
      s_result_error(
  ...
  for (i = 0; i < component_count; ++i)
  {
    ...
    {
      s_result_error(context->result, "failed to enumerate entity component");
      context->ok = false;
      return false;
    }

    if (!s_write_component(context, entity, component_type, &wrote_any_component))
    {
      context->ok = false;
      return false;
    }
    ...
    XStrBuilder *out, LDKGame *game, LDKSceneResult *result)
{
  ...
  if (!out || !game || !game->metadata_count || !game->metadata_get)
  {
  ...
  x_strbuilder_append_format(out, "  version: %u\n", LDK_SCENE_TML_VERSION);
    ...
    LDKScenePendingParentList *parents, LDKSceneResult *result)
{
  ...
  {
    const TMLNode *entity_node = tml_node_child_at(doc, entities_node, i);
    i32 scene_id = LDK_SCENE_NULL_ENTITY_ID;
    i32 parent_id = LDK_SCENE_NULL_ENTITY_ID;
    ...
    if (!s_node_get_i32(doc, entity_node, "entity", &scene_id) || scene_id < 0)
    {
      s_result_error(result, "entity is missing a valid non-negative scene id");
      return false;
    }
    ...
    const LDKComponentMeta *meta, void *component, LDKSceneResult *result)
{
  ...
  for (i = 0; i < fields_node->entry_count; ++i)
  {
    ...
    {
      /*
       * Unknown field: tolerate forward/backward metadata differences.
       */
      continue;
    }
    ...
    const LDKSceneEntityMap *map, LDKSceneResult *result)
{
      ...
      ++entity_index)
  {
    ...
    const TMLNode *components_node = NULL;
    LDKEntity entity = x_handle_null();
    i32 scene_id = LDK_SCENE_NULL_ENTITY_ID;
    ...
    for (component_index = 0; component_index < components_node->child_count;
        ++component_index)
    {
      const TMLNode *component_node =
          tml_node_child_at(doc, components_node, component_index);
      const TMLNode *fields_node = NULL;
      const LDKComponentMeta *meta = NULL;
      u32 component_type = 0;
      void *component = NULL;
      ...
      {
        /*
         * Unknown component type. Tolerate this so old editors can load scenes
         * with newer components without corrupting known data.
         */
        continue;
      }
    ...
    const char *source, LDKGame *game, LDKSceneResult *result)
{
  ...
  TMLDocument *doc = NULL;
  const TMLNode *scene_node = NULL;
  const TMLNode *entities_node = NULL;
  LDKSceneEntityMap map;
  ...
  if (!source || !game || !game->metadata_count || !game->metadata_get)
  {
    ...
    const char *path, LDKGame *game, LDKSceneResult *result)
{
  ...
  if (!ldk_editor_scene_to_tml(builder, game, result))
  {
    ...
    const char *path, LDKGame *game, LDKSceneResult *result)
{
  ...
  ok = ldk_editor_scene_from_tml(text, game, result);
  free(text);
  return ok;
}
