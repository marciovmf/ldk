...
static void __s_editor_project_explorer(
    LDKEditorContext *editor, const char *root_path)
{
  ...
  if (x_fs_path_relative_to(
          &s_root, &s_selected_directory, &relative_selected_directory) > 0)
  {
  ...
  ldk_ui_set_next_weight(ui, 0.0f);
  s_icon_size =
      ldk_ui_slider(ui, s_icon_size, PROJECT_EXPLORER_MIN_ICON_SIZE, 72.0f);
  ...
  if (compact_mode)
  {
    for (u32 entry_i = 0; entry_i < total_count; entry_i++)
    {
      ...
      u32 entry_index =
          is_directory ? entry_i : entry_i - x_array_count(s_dirs);
  ...
  else
  {
    ...
    while (tile_index < total_count)
    {
          ...
          column++, tile_index++)
      {
        ...
        u32 entry_index =
            is_directory ? tile_index : tile_index - x_array_count(s_dirs);
        ProjectExplorerEntry *entry =
            x_array_get(is_directory ? s_dirs : s_files, entry_index);
        ...
        ldk_ui_set_next_size(
            ui, ldk_ui_px(s_icon_size), ldk_ui_px(s_icon_size));
        bool icon_clicked =
            ldk_ui_icon_button(ui, is_directory ? folder_icon : file_icon);
    ...
    LDKEditorContext *editor, const char *root_path)
{
  ...
  while (x_array_count(s_stack) > 0)
  {
    ...
    u32 tree_result = ldk_ui_tree_node_ex(
        ui, node.name.buf, tree_folder_icon, expanded, node.depth, flags);
