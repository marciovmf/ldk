...
bool ldk_mesh_source_set_mesh_index(LDKMeshSource* mesh_source,
    LDKAssetManager* assets, u32 mesh_index)
{
  u32 mesh_count;

  if (!mesh_source || !assets || x_handle_is_null(mesh_source->source_asset.h))
  {
    return false;
  }

  mesh_count = ldk_asset_manager_mesh_count(assets, mesh_source->source_asset);
  if (mesh_index >= mesh_count)
  {
    return false;
  }

  if (mesh_source->mesh_index == mesh_index)
  {
    return ldk_mesh_source_materials_sync(mesh_source, assets);
  }

  mesh_source->mesh_index = mesh_index;
  mesh_source->dirty = true;
  return ldk_mesh_source_materials_sync(mesh_source, assets);
}
    ...
    LDKMeshSource* mesh_source, LDKAssetManager* assets)
{
  ...
  if (!data || mesh_source->mesh_index >=
          ldk_asset_manager_mesh_count(assets, mesh_source->source_asset))
  {
    return false;
  }

  material_count = ldk_asset_manager_mesh_material_slot_count(
      assets, mesh_source->source_asset, mesh_source->mesh_index);
  if (!material_count)
  {
    material_count = 1;
  }
